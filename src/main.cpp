...
// Covers only JSON parsing and sensor-driver call depth now - TLS (network task, HTTP and MQTT alike) and DeviceConfig (heap) are off-stack; [Diag] logs show ~3.5KB real usage, the ~28KB gap to 32768 is a deliberate reserve, not unclaimed headroom to shrink casually.
